...
export const DELIBERATION_RESULT_LABELS: Record<number, string> = {
  0: "En attente",
  1: "Admis",
  2: "Rachat",
  3: "Ajourné",
...
export const DELIBERATION_RESULT = {
  Pending: 0,
  Admis: 1,
  Rachat: 2,
  Ajourne: 3,
...
/** Cycles (L3 or M2 — only final cycles can mint NFTs). */
export function isFinalCycle(cycle: number): boolean {
  return cycle === 0 /* L3 */ || cycle === 1 /* M2 */;
}

// ── Grading scale ────────────────────────────────────────────────────────────
/** Passing grade in centi-points (10.00 / 20). */
export const PASS_GRADE = 1000;
/** Rachat threshold in centi-points (9.90 / 20). */
export const RACHAT_THRESHOLD = 990;
...
/** Batch proposal expiry (in seconds: 7 days). */
export const PROPOSAL_EXPIRY = 7 * 24 * 60 * 60; // 604800 seconds
...
export const CYCLE_OPTIONS = [
  { value: 0, label: "L3 — Licence 3" },
  { value: 1, label: "M2 — Master 2" },
];

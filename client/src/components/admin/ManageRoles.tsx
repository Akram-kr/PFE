...
import { UserPlus, UserMinus, ShieldCheck, Loader2, CheckCircle2, XCircle } from "lucide-react";
import { DIPLOMA_ABI } from "@/lib/abi";
import { CONTRACT_ADDRESS } from "@/lib/wagmi";
import { DEAN_ROLE, RECTOR_ROLE } from "@/lib/contract";
import { cn } from "@/lib/utils";

type RoleAction = "assignDean" | "assignRector" | "removeDean" | "removeRector";
...
function RoleRow({ label, roleHash, assignFn, removeFn, color, onDone }: RoleRowProps) {
  ...
  const { data: hasRoleData } = useReadContract({
    ...
    args: [roleHash, addressValid ? (input as `0x${string}`) : "0x0000000000000000000000000000000000000000"],
  ...
  async function handle(fn: RoleAction) {
    ...
    try {
      ...
      setFeedback({ ok: true, msg: fn.startsWith("assign") ? `${label} assigné avec succès.` : `${label} supprimé.` });
      setInput("");
      onDone();
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-slate-500">
        Saisissez une adresse wallet, puis cliquez sur <strong>Assigner</strong> pour accorder le rôle
        ou <strong>Retirer</strong> pour le révoquer.
      </p>
